...
export class GiftList implements OnInit {
  ...
  PurchaserSrv = inject(PurchasersService);
...
deleteGift(gift: GiftModel) {
  ...
  this.PurchaserSrv.getAll().subscribe({
    next: (purchasers) => {

      const giftPurchasers = purchasers.filter(p => p.giftId === gift.id);

      // אם יש רוכשים → מציג הודעה ועוצר כאן
      if (giftPurchasers.length > 0) {
